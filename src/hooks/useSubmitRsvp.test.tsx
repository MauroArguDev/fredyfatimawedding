...
describe('useSubmitRsvp', () => {
  ...
  it('resolvesWithTheWaLinkOnASuccessfulSubmission', async () => {
    ...
    expect(result.current.data?.waLink).toBe('https://wa.me/50376982534?text=hi');
  });

  it('sendsThePostRequestWithTheTokenAndCountAsJson', async () => {
    vi.mocked(fetch).mockResolvedValue(
      new Response(JSON.stringify({ ok: true, waLink: 'https://wa.me/50376982534' }), {
        status: 200,
      }),
    );
    const { result } = renderHook(() => useSubmitRsvp(), { wrapper: createWrapper() });
