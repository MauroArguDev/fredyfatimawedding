...
> **Regla permanente — nunca emoji en el `text` de un enlace `wa.me`/`api.whatsapp.com` (verificado 2026-09-22).** El servidor de redirección de Meta reemplaza en silencio cualquier carácter de la categoría Unicode "Símbolo" (todo el rango de emoji, sin excepción de cuál ni de qué tan viejo es) por `�` (U+FFFD) al armar la redirección — letras, acentos, ñ, ¡¿ y puntuación normal (incluido el guion largo "—") no se ven afectados, así que no es un problema de codificación ni de fuentes: es un filtro del lado de Meta, ajeno a este código, reproducible con `curl` puro sin pasar por el navegador. No hay combinación de emoji, de codificación (simple o doble `encodeURIComponent`), ni de host (`wa.me` vs `api.whatsapp.com` directo) que lo evite. Cualquier mensaje que se construya para abrirse vía `wa.me`/`api.whatsapp.com` (`src/content/rsvp.ts`, `src/content/adminGuestInvite.ts`) debe ir sin emoji.
