...
**Segundo bug real encontrado al probar `vercel dev` por primera vez (mismo día): pantalla en blanco absoluto en cualquier ruta.** El `rewrite` de `vercel.json` era `/((?!api/.*).*)` → todo lo que no empiece con `api/` cae a `index.html`, pensado para que `/i/abc123` no dé 404 al recargar (WED-12). En producción esto nunca molesta porque Vercel resuelve primero contra los archivos reales de `dist/` y solo cae al rewrite si no hay match. **Pero `vercel dev`, al proxyear hacia el comando `vite` real, aplica el rewrite antes de consultarle a Vite** — así que peticiones internas de Vite como `/src/main.tsx` o `/@vite/client` (que no son "archivos" desde la perspectiva de Vercel) también caían al rewrite y volvían como `index.html` con `Content-Type: text/html`. El navegador rechaza ejecutar HTML como módulo JS → nada se monta → blanco absoluto. **Verificado con curl:** `/src/main.tsx` y `/@vite/client` devolvían el HTML de `index.html`, no el módulo real. **Corregido** cambiando el patrón a `/((?!api/|@|.*\\..*).*)` — excluye además cualquier ruta que empiece con `@` (los módulos virtuales de Vite) o que contenga un punto (cualquier archivo real, `.tsx`/`.js`/`.css`/`.ico`/etc.), dejando el rewrite solo para rutas de la app sin extensión (`/i/:token`, `/admin`, `/styleguide`). Verificado con curl que las 5 combinaciones relevantes siguen correctas: assets de Vite pasan directo, `/` y `/i/:token` caen a `index.html`, `/api/health` sigue devolviendo JSON real. **Efecto secundario positivo:** este patrón también evita que un archivo estático real como `/robots.txt` (WED-14) sea reemplazado por `index.html` si algún día la prioridad archivo-real-antes-que-rewrite de Vercel no aplicara igual — antes dependía enteramente de ese comportamiento implícito, ahora es explícito en la regla misma.
...
**Cobertura.** 287 tests en el repo (antes 235), 98.6 % de cobertura global. `npm run verify` y `npm run build` en verde; el chunk principal de la invitación se mantiene aislado (259.25 kB, prácticamente sin cambio), todo el peso nuevo (RHF + Zod resolver + Dialog) va al chunk de `/admin` (362.67 kB).

**Bug real reportado por el usuario tras probar en local: "el listado de invitados funciona en Vercel pero no en localhost, da error 304".** `fetchAdminApi` solo revisaba `response.ok`, que es `false` para `304 Not Modified` (únicamente 200-299 cuenta como "ok"). **Causa:** `vercel.json` fuerza `Cache-Control: private, no-store` en cada respuesta de `/api/*` en producción — el navegador nunca cachea, así que un 304 no puede ocurrir ahí. En local (el flujo que usa el usuario para levantar `/api/*` junto al front) esa cabecera aparentemente no se replica igual, el navegador termina cacheando la respuesta y en una recarga posterior el servidor contesta 304, que nuestro código trataba como error aunque los datos no habían cambiado. **Corregido** agregando `cache: 'no-store'` a la llamada `fetch()` dentro de `fetchAdminApi` (`src/components/admin/auth/fetchAdminApi.ts`) — esto hace que el propio navegador nunca intente revalidar contra su caché HTTP para ninguna llamada admin, sin depender de que el header del servidor se aplique igual en cada entorno. Cubre las cuatro superficies que pasan por ese helper (listado, crear, editar, eliminar/liberar) de una sola vez. Test nuevo verificando `init.cache === 'no-store'` en cada llamada.

**Gap real de tooling encontrado al verificar este fix: `.lintstagedrc.json` corría `eslint --fix` antes de `prettier --write`.** Como consecuencia, el reformateo de Prettier (que puede partir una línea larga en varias) nunca se volvía a verificar contra reglas sensibles al conteo de líneas como `max-lines-per-function` — así fue como `AdminGuestsBrowser.tsx` y `GuestFormFields.tsx` quedaron por encima del límite de 50 líneas dentro del commit de WED-82 sin que el hook de pre-commit lo detectara. **Corregido invirtiendo el orden** (`prettier --write` primero, `eslint --fix` al final, para que el lint final se aplique sobre el contenido ya formateado) y arreglando los dos archivos que se habían colado (extraídos `GuestField`, `AdminGuestsCrudDialogs` y `AdminGuestsResults` como subcomponentes, reduciendo duplicación de paso). **Patrón a repetir:** después de tocar `eslint.config.ts` o `.lintstagedrc.json`, correr `npm run lint` sobre el repo completo — no alcanza con que el pre-commit hook haya pasado en el momento del commit.
