...
> **Regla permanente — enlaces de WhatsApp siempre por `https://api.whatsapp.com/send/?phone=…&text=…`, nunca por `wa.me` (verificado 2026-09-22; único constructor: `buildWhatsAppDeepLink` en `src/lib/buildWhatsAppDeepLink.ts`, usado por `src/content/rsvp.ts` y por el admin, normaliza el teléfono a solo dígitos).** `wa.me` responde con un 301/302 a `api.whatsapp.com/send/` y en ese paso de redirección del lado del servidor de Meta reemplaza cualquier carácter Unicode de categoría "Símbolo" (cualquier emoji) por `�` (U+FFFD) — reproducible con `curl` puro, sin navegador ni código de este repo de por medio. Yendo directo a `api.whatsapp.com/send/` ese salto no ocurre y el emoji llega intacto al enlace real que abre WhatsApp (`web.whatsapp.com/send/?phone=…&text=…`, visible en el HTML de esa página con los bytes del emoji sin tocar). **Confusión real que costó una vuelta de más:** la página de vista previa de `api.whatsapp.com/send/` sí sigue mostrando `�` en la burbuja de muestra que dibuja para sí misma — es un bug cosmético de esa página nada más, no del enlace real, y fue lo que llevó a la conclusión (incorrecta) de que ningún host servía. La fuente fue esta respuesta, la más votada, de Stack Overflow: https://stackoverflow.com/questions/64215662/whatsapp-click-to-chat-with-emojis (respuesta de Panda Sean). **Segunda mitad de la regla (verificada en dispositivos reales 2026-09-24): solo emoji "sueltos" de un único code point** — nada de modificadores de tono de piel (U+1F3FB–1F3FF, ej. 👋🏼), selector de variación U+FE0F (ej. ☺️) ni ZWJ (U+200D). Con `api.whatsapp.com/send/` y `encodeURIComponent` (UTF-8 estándar, la única codificación que WhatsApp acepta — dobles codificaciones, `%uXXXX`, entidades HTML, escapes JSON y CESU-8 llegan como texto literal o roto) los emoji simples (🥳 ✨ 😊) llegaron intactos al chat, y los compuestos no. Lo hacen cumplir `tests/src/content/rsvp.test.ts` y `tests/src/content/adminGuestInvite.test.ts`.
