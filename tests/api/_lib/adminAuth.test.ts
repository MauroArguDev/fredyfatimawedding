...
import { auth } from '../../../api/_lib/firestore';
import { UnauthorizedError, requireAuth, withAdminAuth } from '../../../api/_lib/adminAuth';
import type { AdminHandler } from '../../../api/_lib/adminAuth';
import type { VercelRequest, VercelResponse } from '@vercel/node';
import type { DecodedIdToken } from 'firebase-admin/auth';

vi.mock('../../../api/_lib/firestore', () => ({ auth: vi.fn() }));
...
function mockVerifyIdToken(implementation: (token: string) => Promise<DecodedIdToken>): void {
  vi.mocked(auth).mockReturnValue({ verifyIdToken: implementation } as unknown as ReturnType<
    typeof auth
  >);
}
...
describe('requireAuth', () => {
  ...
  it('rejectsAnAuthorizationHeaderWithoutTheBearerPrefix', async () => {
    await expect(requireAuth(buildRequest(DECODED_TOKEN.uid))).rejects.toBeInstanceOf(
      UnauthorizedError,
    );
  });
  ...
  it('rejectsWhenFirebaseAdminRejectsTheTokenForAnyReasonIncludingAWrongProjectAudience', async () => {
    ...
    await expect(
      requireAuth(buildRequest('Bearer an-expired-or-foreign-token')),
...
describe('withAdminAuth', () => {
  ...
  it('letsAnUnrelatedErrorFromTheHandlerPropagateInsteadOfSwallowingIt', async () => {
    ...
    await expect(
      withAdminAuth(innerHandler)(buildRequest('Bearer a-valid-token'), response),
