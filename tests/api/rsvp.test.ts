...
import { confirmGuest, findGuestByToken } from '../../api/_lib/guests';
import { isRsvpOpen } from '../../api/_lib/rsvpDeadline';
import { rsvpRateLimiter } from '../../api/_lib/rateLimit';
import handler from '../../api/rsvp';
import type { GuestRecord } from '../../api/_lib/guests';
import type { VercelRequest, VercelResponse } from '@vercel/node';

vi.mock('../../api/_lib/guests', () => ({ confirmGuest: vi.fn(), findGuestByToken: vi.fn() }));
vi.mock('../../api/_lib/rsvpDeadline', () => ({ isRsvpOpen: vi.fn() }));
vi.mock('../../api/_lib/rateLimit', () => ({ rsvpRateLimiter: { shouldLimit: vi.fn() } }));
...
describe('POST /api/rsvp', () => {
  ...
  it('respondsWithOkOnASuccessfulConfirmation', async () => {
    mockConfirmableGuest();
    const response = buildResponse();
